...
impl foxnio::HealthCheck for MockHealthCheck {
    ...
    async fn check(&self) -> anyhow::Result<foxnio::HealthStatus> {
        ...
        if self.healthy {
            Ok(foxnio::HealthStatus::healthy("Check passed", self.latency_ms)
                .with_detail("test_key", "test_value"))
        } else {
            Ok(foxnio::HealthStatus::unhealthy("Check failed", self.latency_ms))
        }
    }
}
...
async fn test_health_checker_basic() {
    ...
    // 注册模拟检查
    checker.register(Box::new(MockHealthCheck {
...
async fn test_health_status_builders() {
    let healthy = foxnio::HealthStatus::healthy("OK", 10)
