...
impl BudgetStatus {
    ...
    pub async fn record_cost(&self, cost_cents: u64) {
        ...
        self.month_used_cents.fetch_add(cost_cents, Ordering::SeqCst);
...
impl CostOptimizer {
    ...
    fn default_provider_pricing() -> HashMap<String, ProviderPricing> {
        ...
        // Anthropic 定价（示例）
        pricing.insert("anthropic".to_string(), ProviderPricing {
            provider: "anthropic".to_string(),
            model_pricing: {
                let mut m = HashMap::new();
                m.insert("claude-3-opus".to_string(), ModelPricing {
                    input_price_per_million: 1500,   // $15
                    output_price_per_million: 7500,  // $75
                });
                m.insert("claude-3-sonnet".to_string(), ModelPricing {
                    input_price_per_million: 300,    // $3
                    output_price_per_million: 1500,  // $15
                });
                m.insert("claude-3-haiku".to_string(), ModelPricing {
        ...
        // OpenAI 定价（示例）
        pricing.insert("openai".to_string(), ProviderPricing {
            provider: "openai".to_string(),
            model_pricing: {
                let mut m = HashMap::new();
                m.insert("gpt-4-turbo".to_string(), ModelPricing {
                    input_price_per_million: 1000,   // $10
                    output_price_per_million: 3000,  // $30
                });
                m.insert("gpt-4".to_string(), ModelPricing {
                    input_price_per_million: 3000,   // $30
                    output_price_per_million: 6000,  // $60
                });
                m.insert("gpt-3.5-turbo".to_string(), ModelPricing {
        ...
        // Gemini 定价（示例）
        pricing.insert("gemini".to_string(), ProviderPricing {
            provider: "gemini".to_string(),
            model_pricing: {
                let mut m = HashMap::new();
                m.insert("gemini-pro".to_string(), ModelPricing {
                    input_price_per_million: 125,    // $1.25
                    output_price_per_million: 375,   // $3.75
                });
                m.insert("gemini-ultra".to_string(), ModelPricing {
    ...
    /// 计算请求成本
    pub fn calculate_cost(&self, provider: &str, model: &str, input_tokens: u64, output_tokens: u64) -> u64 {
        if let Some(pricing) = self.provider_pricing.get(provider) {
            let model_pricing = pricing.model_pricing.get(model)
    ...
    pub fn is_within_budget(&self) -> bool {
        let day_percent = self.budget_status.get_day_usage_percent(self.config.daily_budget_cents);
        let month_percent = self.budget_status.get_month_usage_percent(self.config.monthly_budget_cents);
    ...
    pub fn get_budget_summary(&self) -> BudgetSummary {
        BudgetSummary {
            ...
            month_usage_percent: self.budget_status.get_month_usage_percent(self.config.monthly_budget_cents),
            day_used_cents: self.budget_status.get_day_used(),
            day_budget_cents: self.config.daily_budget_cents,
            day_usage_percent: self.budget_status.get_day_usage_percent(self.config.daily_budget_cents),
    ...
    pub async fn get_optimization_suggestions(&self) -> Vec<OptimizationSuggestion> {
        ...
        for (account_id, records) in history.iter() {
            ...
            let total_tokens: u64 = records.iter().map(|r| r.input_tokens + r.output_tokens).sum();
...
mod tests {
    ...
    async fn test_cost_optimizer() {
        ...
        let recorded_cost = optimizer.record_usage(
            account_id,
            "anthropic",
            "claude-3-opus",
            100_000,
    ...
    async fn test_cost_score() {
        ...
        // 添加一些记录
        optimizer.record_usage(account_id, "anthropic", "claude-3-haiku", 1000, 500).await;
