...
pub async fn update_user_quota(
    ...
    let config = service.update_quota(user_id, req)
        .await
...
pub async fn reset_user_quota(
    ...
    service.reset_quota(user_id)
        .await
        .map_err(|e| ApiError(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;

    Ok(Json(json!({ "success": true, "message": "Quota reset successfully" })))
}
...
pub async fn get_quota_history(
    ...
    let history = service.get_usage_history(user_id, 
...
pub async fn check_quota(
    ...
    let estimated_cost: f64 = body.get("estimated_cost")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);

    let service = QuotaService::new(state.db.clone());
    
    let result = service.check_quota(user_id, estimated_cost)
        .await
        .map_err(|e| ApiError(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
pub async fn consume_quota(
    ...
    let amount: f64 = body.get("amount")
        .and_then(|v| v.as_f64())
        .unwrap_or(0.0);
    let model: &str = body.get("model")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let tokens: i64 = body.get("tokens")
        .and_then(|v| v.as_i64())
        .unwrap_or(0);

    let service = QuotaService::new(state.db.clone());
    
    service.consume_quota(user_id, amount, model, tokens)
        .await
        .map_err(|e| ApiError(StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
